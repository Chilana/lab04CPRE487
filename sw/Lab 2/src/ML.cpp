...
void runLayerTest(const std::size_t layerNum, const Model& model, const Path& basePath, int imageIndex) {
    ...
    try {
        ...
        LayerData img({sizeof(fp32), {64, 64, 3}, basePath / "image_" + std::to_string(imageIndex) + ".bin"});
        img.loadData();
        ...
        std::string imagedata = "image_" + std::to_string(imageIndex) + "_data/";
        Path expectedPath = basePath / imagedata.c_str() / expectedFileName.c_str();
